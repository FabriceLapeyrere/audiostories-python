...
class User(object):
	...
	def build_users(self,entries):
		res=[]
		for e in entries:
			e['verrou']=admin.router.LinkRouter().get_verrou('user/%s' % (e['id']))
			res.append(e)
		return res
	...
	def build_groups(self,entries):
		res=[]
		for e in entries:
			e['verrou']=admin.router.LinkRouter().get_verrou('group/%s' % (e['id']))
			try:
	...
	def del_user(self,userid, iduser):
		if userid!=1 and iduser==1:
			return self.connexion.runQuery('UPDATE users set active=0 WHERE id=?',(userid,)).addCallback(lambda x: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
		return False
	...
	def add_user(self,login,name,password,iduser):
		if iduser==1:
			return self.connexion.runInteraction(self.do_add_user, login, name, password, iduser).addCallback(lambda userid: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
		return False	
	def mod_user(self,userid,login,name,password,role,iduser):
		if iduser==1:
			if password=='':
				return self.connexion.runQuery('UPDATE users set name=?, role=? WHERE id=?',(name,role,userid)).addCallback(lambda x: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
			else:
				password=hashlib.md5(password).hexdigest()
				return self.connexion.runQuery('UPDATE users set name=?, password=?, role=? WHERE id=?',(name,password,role,userid)).addCallback(lambda x: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
		return False
	def mod_moi(self,userid,login,name,password,iduser):
		if iduser==userid:
			if password=='':
				return self.connexion.runQuery('UPDATE users set name=? WHERE id=?',(name,userid)).addCallback(lambda x: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
			else:
				password=hashlib.md5(password).hexdigest()
				return self.connexion.runQuery('UPDATE users set name=?, password=? WHERE id=?',(name,password,userid)).addCallback(lambda x: admin.wsrouter.maj(["user/%s" % userid],userid,iduser))
		return False
	def del_group(self,groupid, iduser):
		if iduser==1:
			return self.connexion.runQuery("DELETE FROM groups WHERE id=?",(groupid,)).addCallback(lambda x: admin.wsrouter.maj(["group/%s" % groupid],groupid,iduser))
		return False
	...
	def add_group(self,nom,iduser):
		if iduser==1:
			return self.connexion.runInteraction(self.do_add_group, nom, iduser).addCallback(lambda groupid: admin.wsrouter.maj(["group/%s" % groupid],groupid,iduser))
	def mod_group(self,groupid,nom,iduser):
		if iduser==1:
			return self.connexion.runQuery('UPDATE groups set nom=? WHERE id=?',(nom,groupid)).addCallback(lambda x: admin.wsrouter.maj(["group/%s" % groupid],groupid,iduser))
		return False
	def add_user_group(self,userid,groupid,iduser):
		if iduser==1:
			return self.connexion.runQuery('INSERT OR REPLACE INTO user_group (id_user,id_group) VALUES (?,?)',(userid,groupid)).addCallback(lambda x: admin.wsrouter.maj(["*"],groupid,iduser))
	def del_user_group(self,userid,groupid,iduser):
		if iduser==1:
			return self.connexion.runQuery('DELETE FROM user_group WHERE id_user=? AND id_group=?',(userid,groupid)).addCallback(lambda x: admin.wsrouter.maj(["*"],groupid,iduser))
	def add_acl(self,type_ressource,id_ressource,type_acces,id_acces,level,iduser):
		return self.connexion.runQuery("INSERT OR REPLACE INTO acl (type_ressource, id_ressource, type_acces, id_acces, level) SELECT ?,?,?,?,? WHERE ?=1 OR (? in (SELECT createdby FROM {} WHERE id=?))".format(type_ressource),(type_ressource,id_ressource,type_acces,id_acces,level,iduser,iduser,id_ressource)).addCallback(lambda x: admin.wsrouter.maj(["*"],iduser,iduser));
	...
	def del_acl(self,type_ressource,id_ressource,type_acces,id_acces,level,iduser):
		return self.connexion.runQuery("DELETE FROM acl WHERE type_ressource=? AND id_ressource=? AND type_acces=? AND id_acces=? AND level=? AND (?=1 OR (? in (SELECT createdby FROM {} WHERE id=?)))".format(type_ressource),(type_ressource,id_ressource,type_acces,id_acces,level,iduser,iduser,id_ressource)).addCallback(lambda x: admin.wsrouter.maj(["*"],iduser,iduser));
