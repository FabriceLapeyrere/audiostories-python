...
def get_page_pub( idpage, iduser):
	return db.connexion.runQuery("SELECT * FROM pages WHERE id=? AND statut=1", (idpage,)).addCallback(lambda x: x[0])
...
def mod_page(params,iduser):
	return db.connexion.runInteraction(this.do_mod_page, params, iduser).addCallback(lambda idpage: maj(["page/%s" % idpage],idpage,iduser))
...
def del_page(params,iduser):
	return db.connexion.runInteraction(this.do_del_page, params, iduser).addCallback(lambda idpage: maj(["page/%s" % idpage],idpage,iduser))
...
def add_page(params,iduser):
	return db.connexion.runInteraction(this.do_add_page, params, iduser).addCallback(lambda idpage: maj(["page/%s" % idpage],idpage,iduser))
